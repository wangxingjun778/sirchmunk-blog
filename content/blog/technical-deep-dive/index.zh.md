...
| **失败模式** | 返回向量索引匹配的内容 — 无补救措施 | 回退到 ReAct 智能体进行迭代深度探索 |

## 3. 系统架构

![Sirchmunk 架构](Sirchmunk_Architecture.png "图 1 — Sirchmunk 高层架构。系统被组织为清晰分离的层次：用于外部接口的集成层、用于搜索管线的编排层、用于证据提取和知识合成的智能层，以及用于持久化的存储层。")
...
- **未找到证据**：系统不会简单放弃。相反，**ReAct 搜索智能体**（详见第 7 节）接管，执行迭代的推理-行动循环以探索替代搜索策略，直到找到答案或耗尽预算。

这种回退到智能体的设计体现了一个核心原则：*系统在承认无知之前应该总是更努力地尝试*。
...
![蒙特卡洛证据采样算法](Sirchmunk_MonteCarloSamplingAlgo.png "图 2 — 蒙特卡洛证据采样：从大文档中提取相关证据的三幕式探索-利用策略。")
...
Top-K 评分片段被传递给 LLM，合成为连贯的兴趣区域（ROI）摘要。输出包含一个置信标志，指示是否发现了强证据，使管线能够做出知情决策：是继续进行摘要，还是回退到 ReAct 智能体。
...
- **自改进智能** — 知识簇随时间积累，使重复查询更快更丰富。
- **优雅降级** — 当标准管线未找到内容时，ReAct 智能体进行更深入的探索而非放弃。
